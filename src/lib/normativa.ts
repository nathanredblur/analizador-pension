...
import {
  ...
  LEY100_SEMANAS_BASE_TASA,
  LEY100_SEMANAS_REQUERIDAS,
  LEY100_TASA_REEMPLAZO_MIN,
  MAX_HIJOS_DESCUENTO,
  SEMANAS_BASE_TASA,
...
export function semanasRequeridas(
  sexo: string,
  anio: number,
  ley: Ley = "ley100",
): number {
  if (ley === "ley100") return LEY100_SEMANAS_REQUERIDAS;
...
export function descuentoSemanasPorHijos(
  ...
  if (ley === "ley100") return baseSemanas;
  const descuento =
...
export function semanasFaltantes(
  ...
  let requeridas = semanasRequeridas(sexo, anio, ley);
  if (sexo === "F") {
    requeridas = descuentoSemanasPorHijos(requeridas, nHijos, ley);
  }
...
export function calcularTasaReemplazo(
  ...
  if (ley === "ley100") {
    const bloques = Math.max(
      0,
      (semanasCotizadas - LEY100_SEMANAS_BASE_TASA) / INCREMENTO_SEMANAS_TASA,
    );
    return Math.min(
      LEY100_TASA_REEMPLAZO_MIN + bloques * INCREMENTO_TASA_POR_BLOQUE,
      TASA_REEMPLAZO_MAX,
    );
  }
  const bloques = Math.max(
...
export function calificaTrasladoRegimen(
  ...
  const cumpleEdad = edad >= edadMin;
  const cumpleSemanas = semanas >= semMin;
  const califica = dentroPlazo && cumpleEdad && cumpleSemanas;
...
export function fechaEstimadaPension(
  ...
  while (anioProyectado <= fechaHoy.getFullYear() + 80) {
    let requeridas = semanasRequeridas(sexo, anioProyectado, ley);
    if (sexo === "F") {
      requeridas = descuentoSemanasPorHijos(requeridas, nHijos, ley);
    }
  ...
  let requeridasFinal = semanasRequeridas(sexo, fechaPension.getFullYear(), ley);
  if (sexo === "F") {
    requeridasFinal = descuentoSemanasPorHijos(requeridasFinal, nHijos, ley);
  }
