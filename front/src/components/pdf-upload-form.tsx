...
export function PdfUploadForm({ onSuccess }: PdfUploadFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!resp.ok) {
        setError(body.error || "Error al procesar el PDF.");
        return;
      }

      const records: CotizacionRecord[] = JSON.parse(body.data);
      onSuccess(records, {
        nombre,
        fecha_nac: fechaNac,
        sexo,
  ...
  return (
    <Card className="w-full max-w-lg">
      <CardHeader>
        <CardTitle className="text-xl">Sube tu reporte de Colpensiones</CardTitle>
        <CardDescription>
          Descarga el PDF desde Mi Colpensiones &rarr; Historial de cotizaciones
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              id="nombre"
              placeholder="Como aparece en el PDF"
              value={nombre}
          ...
          {sexo === "F" && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Ley 2381: descuento de 50 semanas por hijo (máx. 3)
              </p>
            </div>
          )}
